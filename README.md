...
Вот само приложение на потыкать - https://filmbasekinopoisk.vercel.app/ (ВАЖНО! Первый запуск может быть медленным, читать ниже)
...
Из-за этого backend может не отвечать мгновенно после простоя: бесплатные web services на Render **засыпают примерно через 15 минут бездействия** и **просыпаются при следующем HTTP-запросе или WebSocket-подключении**. Из-за этого первый запрос после простоя может открываться с задержкой, поэтому просто через какое-то время перезагрузите страницу
